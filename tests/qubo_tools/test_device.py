...
@dataclass
class _TestCalibration(Calibration):
    """A subclass of Calibration for testing purposes."""

    test_tags: dict[str, list[int]] = field(default_factory=dict)


@pytest.fixture
def sample_device_heavy_hex() -> _TestCalibration:
    ...
    cal.test_tags = {}
    cal.test_tags["longest_hamiltonian_path"] = longest_hamiltonian_path
    cal.test_tags["longest_heavy_chain"] = [
        12,
        31,
        29,
        27,
...
def test_connected_qubit_chain(sample_device_heavy_hex: _TestCalibration) -> None:
    """Tests the computation of the longest hamiltonian path through the device topology.

    Args:
        sample_device_heavy_hex: The testing device calibration.
    """
    chain = sample_device_heavy_hex.get_connected_qubit_chain()
    assert chain == sample_device_heavy_hex.test_tags["longest_hamiltonian_path"]


def test_heavy_chain(sample_device_heavy_hex: _TestCalibration) -> None:
    ...
    assert chain == sample_device_heavy_hex.test_tags["longest_heavy_chain"]


def test_shared_neighbor(sample_device_heavy_hex: _TestCalibration) -> None:
...
def test_from_dict(sample_device_heavy_hex: _TestCalibration) -> None:
