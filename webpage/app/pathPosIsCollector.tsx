...
const PathPosIsCollector: React.FC<PathPosIsCollectorProps> = ({
  title,
  cols = 2,
  allVertices,
  nPaths,
  ...
  const itemAdded = (path: string, position: string, vertex: string) => {
    ...
    if (
      items.some(
        (item) => item[0] == path && item[1] == position && item[2] == vertex,
      )
    )
      return;
    ...
    <div>
      ...
      <div className={`grid grid-cols-${cols} gap-4`}>
        {items.map((item, index) => (
          <PathPosIs
            onClick={(path, position, vertex) =>
              itemRemoved?.(path + "", position + "", vertex)
            }
