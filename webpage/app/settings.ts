...
class Settings {
  ...
  setExactlyOnceVertices(exactlyOnceVertices: boolean[]) {
    const clone = this.clone();
    clone.exactlyOnceVertices = exactlyOnceVertices
      .slice(1)
  ...
  setAtLeastOnceVertices(atLeastOnceVertices: boolean[]) {
    const clone = this.clone();
    clone.atLeastOnceVertices = atLeastOnceVertices
      .slice(1)
      .map((value, index) => (value ? index : -1))
      .filter((value) => value !== -1);
    return clone;
  ...
  setAtMostOnceVertices(atMostOnceVertices: boolean[]) {
    const clone = this.clone();
    clone.atMostOnceVertices = atMostOnceVertices
      .slice(1)
      .map((value, index) => (value ? index : -1))
      .filter((value) => value !== -1);
    return clone;
  ...
  setShareNoVertices(shareNoVertices: boolean[]) {
    const clone = this.clone();
    clone.shareNoVertices = shareNoVertices
      .slice(1)
      .map((value, index) => (value ? index : -1))
      .filter((value) => value !== -1);
    return clone;
  ...
  setShareNoEdges(shareNoEdges: boolean[]) {
    const clone = this.clone();
    clone.shareNoEdges = shareNoEdges
      .slice(1)
      .map((value, index) => (value ? index : -1))
      .filter((value) => value !== -1);
    return clone;
  ...
  clone() {
    ...
    clone.exactlyOnceEdges = this.exactlyOnceEdges.map((value) =>
      value.slice(),
    );
    clone.atLeastOnceEdges = this.atLeastOnceEdges.map((value) =>
      value.slice(),
    );
    ...
    if (this.exactlyOnceEdges.length > 0) {
      constraints.push({
        ...
        path_ids: Array(this.nPaths)
          .fill(0)
          .map((_, index) => index + 1),
        edges: this.exactlyOnceEdges.map((pair) =>
          pair.map((val) => parseInt(val)),
        ),
      });
    }
    if (this.atLeastOnceEdges.length > 0) {
      constraints.push({
        ...
        path_ids: Array(this.nPaths)
          .fill(0)
          .map((_, index) => index + 1),
        edges: this.atLeastOnceEdges.map((pair) =>
          pair.map((val) => parseInt(val)),
        ),
      });
    }
    if (this.atMostOnceEdges.length > 0) {
      constraints.push({
        ...
        path_ids: Array(this.nPaths)
          .fill(0)
          .map((_, index) => index + 1),
        edges: this.atMostOnceEdges.map((pair) =>
          pair.map((val) => parseInt(val)),
        ),
      });
    }
    if (this.pathPositionIs.length > 0) {
      const keyValue: [string[], string][] = this.pathPositionIs.map(
