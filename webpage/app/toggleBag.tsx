...
const ToggleBag: React.FC<ToggleBagProps> = ({
  title,
  items,
  all,
  cols = 4,
  ...
  const [states, setStates] = useState(
    Array(items.length + (all ? 1 : 0)).fill(false),
  );
  useEffect(() => {
  ...
  const update = (index: number, state: boolean) => {
    ...
    if (index == -1 && states.every((state) => state)) newStates.fill(false);
    else if (index == -1) newStates.fill(true);
    ...
    <div>
      ...
      <div className={`grid grid-cols-${cols} gap-4`}>
        {(all ? ["ALL"].concat(items) : items).map((item, index) => (
          <Toggle
            state={states[index]}
            onUpdate={(state) => update(index - (all ? 1 : 0), state)}
            key={index}
          >
            {item}
          </Toggle>
